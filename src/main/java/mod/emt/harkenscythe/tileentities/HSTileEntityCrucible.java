...
import mod.emt.harkenscythe.blocks.HSCrucible;
...
public class HSTileEntityCrucible extends HSTileEntity
{
    ...
    public void setEssenceCount(World world, BlockPos pos, IBlockState state, int essenceCount)
    {
        ...
        int level = (int) Math.ceil((double) getEssenceCount() / MAX_ESSENCE_COUNT * HSCrucible.MAX_LEVEL);
        ((HSCrucible) state.getBlock()).setLevel(world, pos, state, level);
        markDirty();
    }
