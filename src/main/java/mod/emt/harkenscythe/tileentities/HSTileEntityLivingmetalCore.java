...
public class HSTileEntityLivingmetalCore extends HSTileEntity implements ITickable
{
    ...
    public void update()
    {
        ...
        if (state.getValue(HSLivingmetalCore.STATE) == 1)
        {
            ...
            if (!entities.isEmpty())
            {
                ...
                if (this.hasAllies)
                {
                    world.setBlockState(pos, state.withProperty(HSLivingmetalCore.STATE, 2), 3);
                    this.healTimer = 90;
                }
            }
        }
        if (this.healTimer > 0)
        {
            ...
            if (this.healTimer == 0)
            {
                world.setBlockState(pos, state.withProperty(HSLivingmetalCore.STATE, 1), 3);
