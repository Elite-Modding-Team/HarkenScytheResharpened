...
public class HSTileEntityBloodAltar extends TileEntity implements ITickable
{
    ...
    {
        this.bloodCount = scanCrucibleLevels() * 10;
    }
        ...
        for (int dx = -RADIUS; dx <= RADIUS; dx++)
        {
            for (int dy = -RADIUS; dy <= RADIUS; dy++)
            {
                for (int dz = -RADIUS; dz <= RADIUS; dz++)
                {
                    ...
                    if (state.getBlock() instanceof HSBloodCrucible)
                    {
                        int level = state.getValue(HSBloodCrucible.LEVEL);
                        totalLevel += level;
                    }
                }
            }
    ...
    public void decreaseCrucibleLevel(int levelToDecrease)
    {
        ...
        while (remainingLevelToDecrease > 0 && !cruciblePositions.isEmpty())
        {
            ...
            if (state.getBlock() instanceof HSBloodCrucible)
            {
                int currentLevel = state.getValue(HSBloodCrucible.LEVEL);

                if (currentLevel >= remainingLevelToDecrease)
                {
                    world.setBlockState(selectedPos, state.withProperty(HSBloodCrucible.LEVEL, currentLevel - remainingLevelToDecrease));
                    remainingLevelToDecrease = 0;
                }
                else
                {
                    world.setBlockState(selectedPos, state.withProperty(HSBloodCrucible.LEVEL, 0));
